...
<? if($_SESSION['views'] == 0) { ?>
<? include('example.select.project.inc.php');?>
<? if (isset($_SESSION['selectedproject'])){ ?>
<br>
<form action=example.objects.json.php method=post>
  <!--<button type="submit" name="selectSTL" class="btn-sm btn-primary">Manage JSCAD</button> -->
  <button type="submit" name="selectOpenSCAD" class="btn-sm btn-success">OpenSCAD list</button> 
...
<? }?> 
<? if($_SESSION['views'] == 1) { ?>
<? include('example.select.project.inc.php');?><br>
<form action=example.objects.json.php method=post>
...
<? include('display.stl_management.php'); ?> 
<? } ?>
